...
* Roughly how much time you spent on this HW: 10hrs
* What gave you the most trouble: I am very confused with question 2 in general. How much information should I show? How deep I should dig into the data? Very confused. I will add more data analysis in my second submission. 
* Any comments you have: Need to work with others next time...Because my schedule is really off this week I did not get to work with anyone, which made it really hard for me to figure out question two.
...
#I want to get a random selection of the data and then get the rest of the data,
#but don't know how to do so. So just get two random selescted samples for this submission.
samp1<-pollution[sample(nrow(pollution), 30), ] 
samp2<-pollution[sample(nrow(pollution), 30), ] 
...
```
Cross-validation helps us to evaluate the precision of our model, aka. how well it can predicte the correlationship between variables. It helps us to know whether we can apply our model to other data sets and be confident of the predicted results. 
...
```
This sample consists of only 40.2% female. By looking at the age plot and the height plot, we can see that male is on average taller than female, but cannot see a huge difference in terms of age. From the stats table we can see that in our sample, male is on average taller than female and has higher income. Male is on average younger than female by 0.8 year and has a smaller standard deviation.

Ethnicity info:
Ethinicity can be catagorized as white, black, asian, hispanic/latin, and other. But in our dataset many people report themselves belong to multiple ethinicity groups, so I don't know a better way to show ethnicity information.

Education info:
Due to time constraint this time, I plan to dig into education information for my next submission.
...
plot_ori_sex<-ggplot(ori_sex, aes(x=sex, y=n_people, fill=orientation))+
...
```
The status table shows us that in our sample, the proportions of single male and single female are similar. The orientation plot shows that there are proportionally more bisexual female than male. 
